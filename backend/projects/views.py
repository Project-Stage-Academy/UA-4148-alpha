...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.debug(f"Successfully retrieved {len(serializer.data)} projects for user ID: {request.user.id}")
    ...
    def retrieve(self, request, *args, **kwargs):
        logger.info(f"Project retrieve request from user ID: {request.user.id} for project ID: {kwargs.get('pk')}")
        try:
            ...
            logger.debug(f"Successfully retrieved project ID: {kwargs.get('pk')} for user ID: {request.user.id}")
            return Response(serializer.data)
        except Exception as e:
            logger.error(f"Error retrieving project ID {kwargs.get('pk')} for user ID {request.user.id}: {str(e)}", exc_info=True)
            return Response(
    ...
    def subscribe(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            if project.funding_goal is None:
                logger.warning(f"Project subscription failed: project ID {pk} has no funding goal for user ID: {request.user.id}")
                return Response(
            ...
            if project.remaining_funding() < share:
                logger.warning(f"Project subscription failed: funding goal exceeded for project ID {pk} by user ID: {request.user.id}")
                return Response(
        ...
        logger.error(f"Project subscription failed with validation errors for user ID {request.user.id} and project ID {pk}: {serializer.errors}")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    @action(detail=True, methods=["post"])
    def update_project(self, request, pk=None):
        logger.info(f"Project update request from user ID: {request.user.id} for project ID: {pk}")
        try:
            project = StartupProject.objects.get(pk=pk, startup__user=request.user)
        except StartupProject.DoesNotExist:
            logger.warning(f"Project update failed: project ID {pk} not found or access denied for user ID: {request.user.id}")
            return Response(
        ...
        if serializer.is_valid():
            ...
            if changes:
                ...
                logger.info(f"Successfully updated project ID: {pk} with changes: {changes} by user ID: {request.user.id}")
        ...
        logger.error(f"Project update failed with validation errors for user ID {request.user.id} and project ID {pk}: {serializer.errors}")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
