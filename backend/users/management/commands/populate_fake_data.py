...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        projects_per_startup = (
            num_projects // len(startup_profiles) if startup_profiles else 0
        )
        ...
        if num_saved:
            ...
            saved_per_investor = (
                num_saved // len(investor_profiles) if investor_profiles else 0
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Created {saved_created} saved startups.")
        )
